...
@if($aiDraft ?? false)
<div class="mb-4 p-4 bg-purple-50 border border-purple-200 rounded-xl text-sm text-purple-800 flex items-start gap-3">
    <svg class="w-5 h-5 text-purple-500 flex-shrink-0 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 10V3L4 14h7v7l9-11h-7z"/>
    </svg>
    <div>
        <p class="font-semibold mb-0.5">Konten dibuat oleh AI — mohon review sebelum dipublikasikan</p>
        @if(!empty($aiDraft['categories']))
        <p class="text-xs text-purple-600">Kategori yang disarankan: <strong>{{ implode(', ', $aiDraft['categories']) }}</strong></p>
        @endif
    </div>
</div>
@endif
...
<div x-data="articleForm()" class="flex gap-6 items-start">
    ...
    <div class="w-64 flex-shrink-0 space-y-3">
        ...
        {{-- Card 3: Category --}}
        <div class="bg-white rounded-xl border shadow-sm p-4" x-data="categoryPicker()" x-init="fetchAll()">
...
function articleForm() {
    return {
        ...
        init() {
            ...
            const oldContent = {!! json_encode(old('content', ''), JSON_HEX_TAG) !!};
            if (oldContent) {
                const delta = quill.clipboard.convert({ html: oldContent });
                quill.setContents(delta, 'silent');
            }

            @if($aiDraft ?? false)
            // Pre-fill from AI draft
            this.title   = {!! json_encode($aiDraft['title'] ?? '', JSON_HEX_TAG) !!};
            this.excerpt = {!! json_encode($aiDraft['excerpt'] ?? '', JSON_HEX_TAG) !!};
            this.slug    = '';
            this.onTitleInput();
            const aiContent = {!! json_encode($aiDraft['content'] ?? '', JSON_HEX_TAG) !!};
            if (aiContent) {
                const delta = quill.clipboard.convert({ html: aiContent });
                quill.setContents(delta, 'silent');
                this.content = aiContent;
            }
            @endif

            // Trigger reactivity when quill changes (for completeness check)
