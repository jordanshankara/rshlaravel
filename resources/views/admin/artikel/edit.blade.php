    ...
    <div class="bg-white rounded-xl border shadow-sm p-6">
        <form method="POST" action="{{ route('admin.artikel.update', $artikel) }}" enctype="multipart/form-data" class="space-y-5">
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1.5">Cover Image <span class="text-gray-400 font-normal">(kosongkan jika tidak diubah)</span></label>
                    @if($artikel->cover_image)
                    <img src="{{ asset('storage/'.$artikel->cover_image) }}" class="w-24 h-16 object-cover rounded mb-2">
                    @endif
